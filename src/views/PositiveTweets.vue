<template>
    <div class="positive-tweets">
        <b-container>
            <h1 class="text-left mt-3">Positive Tweets</h1>
            <div v-if="error" class="error">{{ error }}</div>
            <div v-if="loading" class="loading">Loading...</div>
            <div v-else>
                <b-form-input id="filter-input" v-model="filter" type="search" placeholder="Type to Search">
                </b-form-input>
                <b-table striped hover :fields="fields" :items="items" @filtered="onFiltered" :filter="filter"
                        :filter-included-fields="filterOn" class="mt-4">
                ...
                <template>
                    <div class="mt-3">
                        ...
                        <b-pagination on-change="changePage" v-model="currentPage" :per-page="perPage"
                            :total-rows="rows" align="center"></b-pagination>
                    </div>
                </template>
            </div>
        </b-container>
...
export default {
    ...
    methods: {
        countTweet() {
            let url = "https://be-sentiment-app-ta.herokuapp.com/tweet/count-positive?hashtag=" + this.hashtag;
            axios
                .get(url)
                .then(
                    function (response) {
        ...
        fetchData(page) {
            let url = "https://be-sentiment-app-ta.herokuapp.com/positive-tweet/all?hashtag=" + this.hashtag + "&page=" + page;
            axios
                .get(url)
                .then(
                    function (response) {
